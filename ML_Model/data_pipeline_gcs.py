...
def preprocess_data_task():
    try:
        ...
        if not blobs:
            raise ValueError(f"No files found in bucket: {GCS_BUCKET} with prefix: {TRAINING_DATA_PREFIX}")
        ...
        print(f"Processed data uploaded to GCS: gs://{GCS_BUCKET}/{processed_blob_name}")
...
def feature_engineering_task():
    try:
        ...
        if not processed_blobs:
            raise ValueError(f"No files found in bucket {GCS_BUCKET} with prefix {PROCESSED_DATA_PREFIX}")
        ...
        print(f"Feature-engineered data uploaded to: gs://{GCS_BUCKET}/{clean_blob_name}")
...
def generate_schema_task():
    try:
        ...
        if not blobs:
            raise ValueError(f"No files found in bucket: {clean_bucket_name} with prefix: {clean_prefix}")
        ...
        schema = {"features": []}  # Include a 'features' key as a list
        for column in df.columns:
            ...
            feature = {
                ...
                "null_count": int(df[column].isnull().sum()),  # Convert to int
                "sample_values": [str(x) for x in df[column].dropna().sample(min(5, len(df[column]))).tolist()],  # Ensure all values are strings
                # Convert NumPy types to Python native types
                "isComplete": bool(df[column].isnull().sum() == 0),  # Example completeness check
                ...
                "monitorAnomalies": True,  # Example monitoring flag
            }
            schema["features"].append(feature)  # Add the feature dictionary to the 'features' list
        ...
        print(f"Schema saved to GCS: gs://{schema_bucket_name}/{schema_blob_name}")
...
def preprocess_new_data_task():
    try:
        ...
        if not blobs:
            raise ValueError(f"No files found in bucket: {GCS_BUCKET} with prefix: {NEW_DATA_PREFIX}")
...
def new_data_feature_selection_task():
    try:
        ...
        if not blobs:
            raise ValueError(f"No processed files found in bucket: {GCS_BUCKET} with prefix: {NEW_DATA_PROCESSED_PREFIX}")
        ...
        if not best_features_blob.exists():
            raise ValueError(f"Best features file not found at: {best_features_blob_path}")
        ...
        print("Loaded training feature set.")
        ...
        if not common_features:
            raise ValueError("No common features found between processed data and training feature set.")
        ...
        clean_data_bucket = client.bucket(NEW_DATA_CLEAN_BUCKET)  # Target the NEW_DATA_CLEAN_BUCKET
        ...
        print(f"Selected features uploaded to GCS: gs://{NEW_DATA_CLEAN_BUCKET}/{selected_features_blob_name}")

        return True

    except ValueError as ve:
        print(f"Feature selection error: {ve}")
        raise
    except Exception as e:
        print(f"Error in new_data_feature_selection_task: {e}")
        raise 
...
def schema_validation_and_anomaly_detection_task():
    try:
        ...
        if not blobs:
            raise ValueError(f"No files found in bucket: {clean_bucket_name}")

        # Identify the newest file
        newest_blob = max(blobs, key=lambda blob: blob.updated)
        print(f"Validating file: {newest_blob.name}")
        gcs_file_path = f"gs://{clean_bucket_name}/{newest_blob.name}"
        ...
        if df.empty:
            raise ValueError("Dataset is empty. Cannot perform schema validation or anomaly detection.")
        ...
        print(f"Attempting to load schema file from: gs://{schema_bucket_name}/{schema_blob_path}")
        schema_bucket = client.bucket(schema_bucket_name)
        schema_blob = schema_bucket.blob(schema_blob_path)

        if not schema_blob.exists():
            raise ValueError(f"Schema configuration file not found at: {schema_blob_path}")

        schema_config = json.loads(schema_blob.download_as_text())
        print("Schema configuration loaded from GCS.")

        # Validate schema structure
        if "features" not in schema_config or not schema_config["features"]:
            raise ValueError("Schema configuration is invalid or missing 'features'.")
        ...
        for feature in schema_config["features"]:
            ...
            if column_name not in df.columns:
                print(f"Warning: Column {column_name} is missing in the dataset. Skipping validation for this column.")
                continue
            ...
            if df[column_name].dtype in ["int64", "float64"]:
                if "hasMin" in feature:
                    if (df[column_name] < feature["hasMin"]).any():
                        raise ValueError(f"Column {column_name} has values below the minimum allowed.")
                if "hasMax" in feature:
                    if (df[column_name] > feature["hasMax"]).any():
                        raise ValueError(f"Column {column_name} has values above the maximum allowed.")
        ...
        for feature in schema_config["features"]:
            if feature.get("monitorAnomalies", False):
                ...
                if df[column_name].dtype in ["int64", "float64"]:
                    ...
                    if anomalies.size > 0:
                        print(f"Anomalies detected for {column_name} at indices: {anomalies.tolist()}")
                    else:
                        print(f"No anomalies detected for {column_name}.")
        ...
        print(f"Error in schema_validation_and_anomaly_detection_task: {e}")
        raise
...
def convert_latest_csv_to_jsonl_task():
    try:
        ...
        if not blobs:
            raise ValueError(f"No files found in bucket {NEW_DATA_CLEAN_BUCKET}")

        # Find the latest file by updated timestamp
        latest_blob = max(blobs, key=lambda x: x.updated)
        source_file_name = latest_blob.name
        print(f"Latest file selected: {source_file_name}")
        ...
        print(f"Downloaded latest file to {local_csv_path}")
        ...
        print(f"DataFrame converted to JSONL format and saved at {local_jsonl_path}")
        ...
        print(f"JSONL file uploaded to bucket {destination_bucket_name} as {destination_file_name}")
        ...
        print(f"Error in convert_latest_csv_to_jsonl_task: {e}")
        raise
